...
两只 96×96 像素风小猫轮班：**Midi**（虎斑，白班 8–20 点）和 **咖啡**（暹罗，其余时间）；换班时当班的走出画面、接班的从灯后面走进来。
F 页场景下方的「换班」按钮让另一只现在来接班；托盘「Midi 形象」可以固定某一只。动作相同：左右爪打字、抬头、眨眼、动耳朵、夜间眯眼和蜷睡。
实现与截图见 [暹罗记录](design/midi-cat-v3/IMPLEMENTATION.md) 与 [虎斑记录](design/midi-cat-v2/IMPLEMENTATION.md)。
